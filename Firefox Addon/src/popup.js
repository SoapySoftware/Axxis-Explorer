'use strict';

import './popup.css';

(function () {
  console.log('Popup.js script loaded');

  const appName = 'com.marcon.axxis.explorer';

  const appController = (request, sendResponse) => {
    let app = browser.runtime.connectNative(appName);

    // Handle the response from the native app.  Will let us know if the parameters sent were valid.
    app.onMessage.addListener(function (msg) {
      console.log('Received message from app (popup.js)', msg);
      sendResponse(msg);
    });

    // Handle disconnection.
    // Application only handles one request, then exits.
    // We must check the lastError message, otherwise it appears that the extension is throwing errors.
    app.onDisconnect.addListener(function () {
      console.log(
        'Disconnected from app (popup.js):',
        browser.runtime.lastError
      );

      const msg = browser.runtime.lastError
        ? browser.runtime.lastError.message
        : '';

      if (
        msg.startsWith('Specified native messaging host not found.') ||
        msg.startsWith(
          'Access to the specified native messaging host is forbidden.'
        )
      ) {
        // The native app is not installed
        request.status = 'error';
        request.message = 'Native app is not installed';
        sendResponse(request);
      }

      app = null;
    });

    // Send the request to the native app.
    console.log('Sending message to app (popup.js)', request);
    app.postMessage(request);
  };

  browser.permissions.getAll().then((permissions) => {
    console.log('Permissions granted (popup.js): ', permissions);

    let elem = document.getElementById('txtStatusFileUrls');

    if (
      permissions.origins.includes('*://*.marconmetalfab.com/*') &&
      permissions.origins.includes('*://axx.is/*')
    ) {
      elem.innerText = 'Success';
      elem.classList.remove('text-muted');
      elem.classList.add('text-success');
    } else {
      elem.innerHTML =
        'Failed <a href="help/allow-access.html" target="_blank" title="Help setting up access to allow local file URLs"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-question-octagon-fill" viewBox="0 0 16 16"><path d="M11.46.146A.5.5 0 0 0 11.107 0H4.893a.5.5 0 0 0-.353.146L.146 4.54A.5.5 0 0 0 0 4.893v6.214a.5.5 0 0 0 .146.353l4.394 4.394a.5.5 0 0 0 .353.146h6.214a.5.5 0 0 0 .353-.146l4.394-4.394a.5.5 0 0 0 .146-.353V4.893a.5.5 0 0 0-.146-.353zM5.496 6.033a.237.237 0 0 1-.24-.247C5.35 4.091 6.737 3.5 8.005 3.5c1.396 0 2.672.73 2.672 2.24 0 1.08-.635 1.594-1.244 2.057-.737.559-1.01.768-1.01 1.486v.105a.25.25 0 0 1-.25.25h-.81a.25.25 0 0 1-.25-.246l-.004-.217c-.038-.927.495-1.498 1.168-1.987.59-.444.965-.736.965-1.371 0-.825-.628-1.168-1.314-1.168-.803 0-1.253.478-1.342 1.134-.018.137-.128.25-.266.25h-.825zm2.325 6.443c-.584 0-1.009-.394-1.009-.927 0-.552.425-.94 1.01-.94.609 0 1.028.388 1.028.94 0 .533-.42.927-1.029.927"/></svg></a>';

      if (!permissions.origins.includes('*://*.marconmetalfab.com/*')) {
        elem.innerHTML += '<br/>Missing: marconmetalfab.com';
      }
      if (!permissions.origins.includes('*://axx.is/*')) {
        elem.innerHTML += '<br/>Missing: axx.is';
      }

      elem.classList.remove('text-muted');
      elem.classList.add('text-danger');
    }
  });

  appController({ type: 'checkInstalled' }, (response) => {
    console.log('Response from checkInstalled (popup.js)', response);
    let elem = document.getElementById('txtStatusApp');
    if (response.status === 'success') {
      elem.innerText = 'Success';
      elem.classList.remove('text-muted');
      elem.classList.add('text-success');
    } else {
      elem.innerHTML =
        'Failed <a href="help/install.html" target="_blank" title="Help installing the helper app"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-question-octagon-fill" viewBox="0 0 16 16"><path d="M11.46.146A.5.5 0 0 0 11.107 0H4.893a.5.5 0 0 0-.353.146L.146 4.54A.5.5 0 0 0 0 4.893v6.214a.5.5 0 0 0 .146.353l4.394 4.394a.5.5 0 0 0 .353.146h6.214a.5.5 0 0 0 .353-.146l4.394-4.394a.5.5 0 0 0 .146-.353V4.893a.5.5 0 0 0-.146-.353zM5.496 6.033a.237.237 0 0 1-.24-.247C5.35 4.091 6.737 3.5 8.005 3.5c1.396 0 2.672.73 2.672 2.24 0 1.08-.635 1.594-1.244 2.057-.737.559-1.01.768-1.01 1.486v.105a.25.25 0 0 1-.25.25h-.81a.25.25 0 0 1-.25-.246l-.004-.217c-.038-.927.495-1.498 1.168-1.987.59-.444.965-.736.965-1.371 0-.825-.628-1.168-1.314-1.168-.803 0-1.253.478-1.342 1.134-.018.137-.128.25-.266.25h-.825zm2.325 6.443c-.584 0-1.009-.394-1.009-.927 0-.552.425-.94 1.01-.94.609 0 1.028.388 1.028.94 0 .533-.42.927-1.029.927"/></svg></a>';
      elem.setAttribute('title', JSON.stringify(response));
      elem.classList.remove('text-muted');
      elem.classList.add('text-danger');
    }
  });

  // Helper function to open a new tab
  const openTab = (url, callback = () => {}) => {
    browser.tabs.create(
      {
        url,
      },
      callback
    );
  };
})();
